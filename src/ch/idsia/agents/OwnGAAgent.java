...
//		System.out.println("enemies : "+probe(1,0,enemies));
	...
	public int calc_cost() {
		int res = 0;
		int x = this.marioEgoCol;
		int y = this.marioEgoRow;
		int dx[] = {-1,1,0,0};
		int dy[] = {0,0,1,-1};
		...
		while(q.size()!=0) {
			...
			for(int i=0;i<4;i++) {
				...
				if(getReceptiveFieldCellValue(nx, ny)==-60||getReceptiveFieldCellValue(nx, ny)==-24) continue;
				if(dist[nx][ny]>dist[xx][yy]+1) {
					dist[nx][ny] = dist[xx][yy]+1;
					Point ne = new Point(nx,ny);
					q.add(ne);
				}
			}
