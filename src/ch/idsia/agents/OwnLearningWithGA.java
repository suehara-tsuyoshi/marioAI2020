...
public class OwnLearningWithGA implements LearningAgent{
	...
	final float bigMutateRate = 0.5f;
	final float crossRate = 1.0f;

//	private LearningTask task = null;
	private String name = "OwnLearningWithGA";
	private OwnGAAgent[] agents;
...
//	private EvaluationInfo localEvaluationInfo;
	...
	public OwnLearningWithGA(String args){
	...
	public void learn(){
		...
		for(int generation=0 ; generation<evaluationQuota ; generation++){
			...
			/*int bigMutatePeriod = 50;
			if(generation%bigMutatePeriod==0) bigMutate();
			else mutate();*/
	...
	private void compFit() {
		...
		for(int i=0 ; i<popsize ; i++){
			...
			//int sub = agents[i].calc_cost();

			agents[i].setFitness(evaluationInfo.distancePassedCells);
			agents[i].setDist(evaluationInfo.distancePassedPhys);
		...
		System.out.println("agents[0]Fitness : "+presentBestAgentDistance +"\n"+ "agents[0].distance : " + agents[0].getDistance()
				+"\n"+ "agents[0].distance2 : " + agents[0].getDist());
...
//		float mutation3 = (1 << 16) * mutateRate;	//突然変異させる遺伝子座の個数(float型)
	...
	private void bigMutate() {

		int popsize2 = popsize - 2;
//		float mutation = popsize * mutateRate;	//突然変異させる個体数(float型)を設定(現在10個体)
		int mutationInt = (int) Math.floor(popsize*bigMutateRate);	//突然変異させる個体数(int型)
//		float mutation3 = (1 << 16) * mutateRate;	//突然変異させる遺伝子座の個数(float型)
		int mutateGeneInt = (int) Math.floor((1<<16) * bigMutateRate);	//突然変異させる遺伝子座の個数(int型)(65536)
