...
public class OwnMCAgent extends BasicMarioAIAgent implements Agent{
	...
	public static HashMap<OwnKeyOfMC,Integer> selected;
	//行動価値関数　これを基に行動を決める
	...
	*/
	...
	public OwnMCAgent(){
		...
		selected = new HashMap<OwnKeyOfMC,Integer>();
		for(int i = 0; i < (int)Math.pow(2.0,4 * width + 1); ++i){
			for(int j = 0; j < 2; ++j){
				for(int k = 0; k < 2; ++k){
					for(int t = 0; t < numOfAction; ++t){
	...
	public void intToAction(int n){
		...
		*/
		...
		action[Mario.KEY_SPEED] = true;
		action[Mario.KEY_JUMP] = (n==0||n==1);
		action[Mario.KEY_RIGHT] = (n==0||n==2);
		action[Mario.KEY_LEFT] = (n==1||n==3);
	...
	public boolean[] getAction(){
		detectObstacle();
		detectCliff();
		ableToJump = boolToInt(!isMarioOnGround||isMarioAbleToJump);
		clearAction();
		int currAction = 0;
		if(!mode){
			...
			if(!selected.containsKey(new KeyOfMC(state,cliff,ableToJump,currAction)))
				selected.put(new OwnKeyOfMC(state,cliff,ableToJump,currAction),1);
			else
				selected.put(new OwnKeyOfMC(state,cliff,ableToJump,currAction), selected.get(new KeyOfMC(state,cliff,ableToJump,currAction)) + 1);
		}
		else{
